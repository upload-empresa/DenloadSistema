...
export default function FotoSettings() {
    const router = useRouter();

    // TODO: Undefined check redirects to error
    const { id: fotoId } = router.query;

    const { data: settings, isValidating } = useSWR<WithPacienteFoto>(
        `/api/foto?fotoId=${fotoId}`,
        fetcher,
        {
            onError: () => router.push("/"),
    ...
    async function saveFotoSettings(data: SettingsData) {
        setSaving(true);
    ...
    async function deleteFoto(fotoId: string) {
        ...
        try {
            const response = await fetch(`/api/foto?fotoId=${fotoId}`, {
    ...
    return (
        <>
            <Layout
                //@ts-ignore 
                pacienteId={settings?.paciente?.id}>
                <Toaster
                    position="top-right"
                    toastOptions={{
                        duration: 10000,
                ...
                <div className="max-w-screen-xl mx-auto px-10 sm:px-20 mt-20 mb-16">
                    <h1 className="font-cal text-5xl mb-12">Foto Settings</h1>
                    <div className="mb-28 flex flex-col space-y-12">
                        <div className="space-y-6">
                            <h2 className="font-cal text-2xl">Foto Slug</h2>
                        ...
                        <div className="space-y-6">
                            ...
                            <div
                                //@ts-ignore
                                className={`${data.foto ? "" : "animate-pulse bg-gray-300 h-150"
                            ...
                            <div className="flex flex-col space-y-6 max-w-lg">
                                <h2 className="font-cal text-2xl">Delete Foto</h2>
                                <p>
                                    Permanently delete your Foto and all of its contents from our
                ...
                <Modal showModal={showDeleteModal} setShowModal={setShowDeleteModal}>
                    <form
                        onSubmit={async (event) => {
                            ...
                            await deleteFoto(fotoId as string);
                        }}
                        className="inline-block w-full max-w-md pt-8 overflow-hidden text-center align-middle transition-all bg-white shadow-xl rounded-lg"
                    >
                        <h2 className="font-cal text-2xl mb-6">Delete foto</h2>
                        <div className="grid gap-y-5 w-5/6 mx-auto">
                            <p className="text-gray-600 mb-3">
                                Are you sure you want to delete your foto? This action is not
                                reversible.
                            </p>
                        </div>
                        <div className="flex justify-between items-center mt-10 w-full">
                            ...
                            <button
                                ...
                                className={`${deletingFoto
                                    ? "cursor-not-allowed text-gray-400 bg-gray-50"
                                    : "bg-white text-gray-600 hover:text-black"
                                    } w-full px-5 py-5 text-sm border-t border-l border-gray-300 rounded-br focus:outline-none focus:ring-0 transition-all ease-in-out duration-150`}
                            >
                                {deletingFoto ? <LoadingDots /> : "DELETE POST"}
                            </button>
                        </div>
                    </form>
                </Modal>
