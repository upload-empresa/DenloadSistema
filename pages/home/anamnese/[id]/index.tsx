...
export default function Anamnese() {
    ...
    const { id: anamneseId } = router.query;

    const { data: anamnese, isValidating } = useSWR<WithPacienteAnamnese>(
        router.isReady && `/api/anamnese?anamneseId=${anamneseId}`,
        fetcher,
        {
    ...
    const [savedState, setSavedState] = useState(
        anamnese
            ? `Last saved at ${Intl.DateTimeFormat("en", { month: "short" }).format(
                new Date(anamnese.updatedAt)
            )} ${Intl.DateTimeFormat("en", { day: "2-digit" }).format(
                new Date(anamnese.updatedAt)
            )} ${Intl.DateTimeFormat("en", {
                hour: "numeric",
                minute: "numeric",
            }).format(new Date(anamnese.updatedAt))}`
            : "Saving changes..."
    );
    ...
    useEffect(() => {
        if (anamnese)
            setData({
                pergunta: anamnese.pergunta ?? "",
                resposta: anamnese.resposta ?? "",
            });
    }, [anamnese]);
    ...
    useEffect(() => {
        if (debouncedData.pergunta) saveChanges(debouncedData);
    ...
    async function publish() {
        ...
        try {
            ...
            if (response.ok) {
                mutate(`/api/anamnese?anamneseId=${anamneseId}`);
    ...
    return (
        <>
            ...
            <Layout
                //@ts-ignore
                pacienteId={anamnese?.paciente?.id}>
                <div className="max-w-screen-xl mx-auto px-10 sm:px-20 mt-10 mb-16">
                    <TextareaAutosize
                        ...
                        onInput={(e: ChangeEvent<HTMLTextAreaElement>) =>
                            setData({
                                ...data,
                                pergunta: (e.target as HTMLTextAreaElement).value,
                            })
                        }
                        ...
                        placeholder="Unperguntad anamnese"
                        value={data.pergunta}
                    />
                    <TextareaAutosize
                        name="resposta"
                        onInput={(e: ChangeEvent<HTMLTextAreaElement>) =>
                            setData({
                                ...data,
                                resposta: (e.target as HTMLTextAreaElement).value,
                            })
                        }
                        ...
                        placeholder="No resposta provided. Click to edit."
                        value={data.resposta}
                    />
                ...
                <footer className="h-20 z-5 fixed bottom-0 inset-x-0 border-solid border-t border-gray-500 bg-white">
                    <div className="max-w-screen-xl mx-auto px-10 sm:px-20 h-full flex justify-between items-center">
                        <div className="text-sm">
                            <strong>
                                <p>{anamnese?.published ? "Published" : "Draft"}</p>
                            </strong>
                            <p>{savedState}</p>
                        </div>
                        <button
                            ...
                            pergunta={
                                disabled
                                    ? "anamnese must have a pergunta, resposta, and content to be published."
                                    : "Publish"
                            }
                            disabled={disabled}
                            className={`${disabled
