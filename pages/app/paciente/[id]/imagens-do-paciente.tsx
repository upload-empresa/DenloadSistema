...
export default function DocPacientes() {
    ...
    const { data: fotoMostrar } = useSWR<SiteFotoData>(
    ...
    async function createFoto(pacienteId: string) {
        try {
            ...
            if (res.ok) {
                ...
                router.push(`/foto/${data.fotoId}/imagens-do-paciente`);
            }
        } catch (error) {
            console.error(error);
        }
            ...
            >
                ...
                <CardMain radius={'0 18px 18px 0'} spacing={5} w={"90%"}>
                    ...
                    >
                        ...
                        {/* <CldUploadButton uploadPreset="nextjs-clduploadbutton-unsigned" on={handleOn} /> */}
                        {error && <p>{error.status}</p>}
                        {info && (
                            <>
                                {info.resource_type === 'image' && (
                                    <p><Image width={info.width} height={info.height} src={info.secure_url} alt="Uploaded image" /></p>
                                )}
                                {info.resource_type === 'video' && (
                                    <p><video controls width={info.width} height={info.height} src={info.secure_url} /></p>
                                )}
                                <p>{info?.secure_url}</p>
                            </>
                        )}          </Stack>
